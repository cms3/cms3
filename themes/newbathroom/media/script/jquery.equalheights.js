...
function equalHeights_sub(element) {	
		var class_name_sub = '.equal-height_sub';
		var maxHeight_sub = 0;	
		var maxItem_sub = 0;
		var maxItem_sub_adjust = 0;		

		element.find(class_name_sub).each(function(j){	
			$(this).wrapInner('<div></div>');				
		});	
				
		function heightChecker_sub(){			
			element.find(class_name_sub).each(function(j){						
				if($(this).outerHeight() > maxHeight_sub) { 
						maxHeight_sub = $(this).outerHeight();
						maxItem_sub = $(this);
						setHeight_sub(maxHeight_sub);
					}				
			});				
			
			maxItem_sub_adjust = maxItem_sub.outerHeight()-maxItem_sub.height();
			if(maxHeight_sub > (maxItem_sub.children().height() + maxItem_sub_adjust)){
					maxHeight_sub = maxItem_sub.children().height() + maxItem_sub_adjust;
					setHeight_sub(maxHeight_sub);									
				}
				
		//	setTimeout(function(){heightChecker_sub()}, 1000)			
		 }	
		 
		function setHeight_sub(target){
			element.find(class_name_sub).each(function(j){
						var adjust_sub = $(this).outerHeight()-$(this).height();						
						if($.browser.msie && $.browser.version < 7.0){
								   $(this).css({'height': target - adjust_sub });								   
							}
							else{
  								   $(this).css({'min-height': target - adjust_sub });	
								}
					})
		  }
		heightChecker_sub();		  
