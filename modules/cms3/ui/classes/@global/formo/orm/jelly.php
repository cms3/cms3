...
class Formo_ORM_Jelly extends Formo_ORM {
	 ...
	 * @static
	 */
	protected static $relationship_types = array(
		'Field_HasMany' 	=> 'has_many',
		'Field_BelongsTo'	=> 'belongs_to',
		'Field_HasOne'		=> 'has_one',
		'Field_ManyToMany'	=> 'many_to_many'
	);
	...
	public function load(\Jelly_Model $model, array $fields = NULL, $skip_fields = FALSE)
	{
		...
		{
			if ($this->use_field($field->name) === FALSE)
				// If the field is supposed to be skipped, ignore it altogether
				continue;
				
			if (isset($this->has_many['definitions'][$field->name]) || isset($this->has_one['definitions'][$field->name]))
			{
				continue;
			}
			...
			if ($options['value'] instanceof \Jelly_Model)
			{
				$options['value'] = $options['value']->id();
			}
			elseif ($options['value'] instanceof \Jelly_Collection)
			{
				$options['value'] = $options['value']->as_array($model->meta()->primary_key(), $model->meta()->primary_key());
			}
			
			// If the field is a belongs_to field, do some extra processing
			$foreign_key = $this->process_belongs_to($field->name, $options);
			/*
			$this->process_has_many($field->name, $options);
			...
			*/
			...
			$this->add_meta($field->name, $options, $foreign_key);
	...
	public function set_field(Formo_Container $field, $value)
	{
		...
		elseif ($definitions)
		{
			...
			{
				$record = Jelly::model($definitions['model'], $_value);
	...
	//	$_alias = $this->add_alias($field_alias, $options);
		...
		$model = $this->belongs_to['definitions'][$alias]->foreign['model'];
	...
	protected function process_has_many($alias, array & $options)
	{
		if (isset($this->has_many['definitions'][$alias]))
		{
			$model = $this->has_many['definitions'][$alias]->foreign['model'];
			$model = \CMS3\Engine\Model::factory($model);
			$this->relational_fields[$alias] = $model;
			$query = $model->query();
	...
	protected function add_has_one()
	{
		...
		foreach ($this->has_one['definitions'] as $alias => $value)
		{
			...
			$this->relational_fields[$_alias] = Jelly::model($this->has_one['definitions'][$alias]['model']);
		}
	...
	protected function add_has_relationships()
	{
		foreach (array('has_many', 'has_one') as $type)
		{
			foreach ($this->{$type}['definitions'] as $alias => $value)
			{
				...
				$model = $this->{$type}['definitions'][$alias]->foreign['model'];
				$this->relational_fields[$_alias] = \CMS3\Engine\Model::factory($model);
	 ...
	 * @return void
	 */
	protected function add_options($alias, \Jelly_Builder $query, array & $options)
	{
		/*
...
*/
		...
		{
			$primary_key = $row->meta()->primary_key();

			if ($row->{$row->meta()->name_key()})
			{
				$name = $row->{$row->meta()->name_key()};
			}
			else
			{
	...
	{
		foreach ($this->model->meta()->fields() as $field)
		{
			$field_type = \CMS3\Engine\NS::extract_class_name(get_class($field)); 
			if (isset(self::$relationship_types[$field_type]))
			{
				$alias = self::$relationship_types[$field_type];
				$this->{$alias}['definitions'][$field->name] = $field;
				$this->{$alias}['foreign_keys'][$field->name] = $field->foreign;
			}
		}
	...
	public function save_rel()
	{
		...
		foreach ($this->has_many_relationships as $values)
		{
			...
			$remove_query = DB::update($table_name)
				...
				->where($foreign_key, '=', $this->model->id());
			...
			if ($values['value'])
			{
				...
				$add_query = DB::update($table_name)
					->set(array($foreign_key => $this->model->id()))
	...
	public function pre_render()
	{
		...
		foreach ($this->relational_fields as $alias => $model)
		{
