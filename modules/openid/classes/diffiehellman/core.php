 ...
 *
 * $Id: diffiehellman.php 2008-09-25 17:28:07 BST Atomless $
 *
 * @package    Openid
 * @author     Kohana Team
...
class diffiehellman_Core {
	...
	{
		$modulus = ($modulus === FALSE)? diffiehellman::DEFAULT_MODULUS : $modulus;

		$generator = ($generator === FALSE)? diffiehellman::DEFAULT_GENERATOR : $generator;
	 ...
	 */
	public static function compute_mac_key($dh_server_public = FALSE, $enc_mac_key = FALSE, $assoc_type = FALSE, $private = FALSE, $modulus = FALSE)
	{
		...
		return base64_encode(diffiehellman::xor_mac_key($dh_server_public, $enc_mac_key, $hash_type, $private, $modulus));
	}
	...
	{
		$dh_mac_key = diffiehellman::get_mac_key($composite, $private, $modulus);

		$dh_mac_key_str = crypt::long_to_binary($dh_mac_key);

		if ( ! in_array(strtolower($hash_type), hash_algos()))
			throw new Kohana_Exception('diffiehellman.unsupported_hash_type', $hash_type);
