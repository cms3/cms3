 ...
 *
 * $Id: xrds.php 2008-09-25 17:28:07 BST Atomless $
 *
 * @package    Openid
 * @author     Kohana Team
 ...
 */
class Openid_Service_xrds {

	// This match mode means a given service must match ALL filters to
	// be returned by the openid_service_xrds::filtered_services() method.
	const MATCH_ALL = 'match all filters';

	// This match mode means a given service must match ANY filters (at
	// least one) to be returned by the openid_service_xrds::filtered_services() method.
	const MATCH_ANY = 'match any filter';

	// List of valid filter modes
	public static $valid_filter_modes = array
	(
		openid_service_xrds::MATCH_ALL,
		openid_service_xrds::MATCH_ANY
	);

	/**
	 * Callback filter function used by openid_service_xrds::get_filtered_xrds_services.
	 ...
	 */
	public static function get_filtered_xrds_services(array $xrds_services, $filters = FALSE, $filter_mode = openid_service_xrds::MATCH_ALL)
	{
		...
		if ( ! in_array($filter_mode, openid_service_xrds::$valid_filter_modes))
			return FALSE;
		...
		foreach ($xrds_services as $priority_key => $service_with_this_priority)
		{
			foreach ($service_with_this_priority as $service)
			{
				...
				{
					if (openid_service_xrds::$filter($service))
					{
						$matches++;

						if ($filter_mode == openid_service_xrds::MATCH_ANY)
						{
				...
				if (($filter_mode == openid_service_xrds::MATCH_ALL) AND ($matches == count($filters))) {
	 ...
	 * @param  string   verbatum copy of the url/id entered by the user into the openid_identifier field
	...
	public static function get_filtered_and_prioritized_service_endpoints($xrds_services, $discovered_claimed_id, $user_supplied_id)
	{
		...
		$filtered_services = openid_service_xrds::get_filtered_xrds_services($xrds_services, $filters);

		if (empty($filtered_services))
			return FALSE;

		$services_endpoints = openid_service_xrds::get_openid_service_endpoints($filtered_services, $discovered_claimed_id, $user_supplied_id);

		if (empty($services_endpoints))
			return FALSE;

		$services_endpoints = openid_service_xrds::get_prioritised_openid_provider_or_user_service_endpoints($services_endpoints);
	...
	public static function get_openid_service_endpoints($xrds_services = array(), $discovered_claimed_id, $user_supplied_id)
	{
		...
		foreach ($xrds_services as $services)
		{
			foreach($services as $xrds_service)
			{
				...
				{
					$openid_service_endpoint = openid_service_xrds::to_openid_service_endpoint
					(
						$xrds_service,
	...
	public static function to_openid_service_endpoint($xrds_service, $discovered_claimed_id, $user_supplied_id, $server_url, $type_uris)
	{
		...
		if ( ! $openid_service_endpoint->is_openid_provider_type())
		{
			...
			$local_id = openid_service_xrds::find_openid_provider_local_identifier($xrds_service, $type_uris);
	...
	{
		$openid_provider_service_endpoints = openid_service_xrds::arrange_by_type($openid_service_endpoints, array(Openid::OPENID_2_0_NAMESPACE_SERVER));

		$openid_service_endpoints = openid_service_xrds::arrange_by_type($openid_service_endpoints, Openid::$yadis_service_types);
	...
	public static function arrange_by_type($openid_service_endpoints, $preferred_types = array())
	{
		...
		foreach ($openid_service_endpoints as $index => $openid_service_endpoint)
		{
			array_push(
						...
						array(
							  openid_service_xrds::index_of_best_matching_service($openid_service_endpoint, $preferred_types),
							  $index,
