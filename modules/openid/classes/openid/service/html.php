 ...
 *
 * $Id: html.php 2008-09-25 17:28:07 BST Atomless $
 *
 * @package    Openid
 * @author     Kohana Team
...
class Openid_Service_html {
	...
	public static function get_openid_service_endpoints($html_content, $claimed_id)
	{
		...
		foreach (openid_service_html::$openid_html_discovery_versions as $discovery_version) {

			$Openid_identifiers = openid_service_html::get_openid_server_and_local_urls
			(
				$html_content,
			...
			if ($Openid_identifiers === FALSE)
				continue;

			if ($Openid_identifiers['local_url'] === FALSE)
			{
				$Openid_identifiers['local_url'] = $claimed_id;
			}

			$service_endpoint_fields = array
			(
				'claimed_id'       => $claimed_id,
				'identity'         => $Openid_identifiers['local_url'],
				'display_id' => $claimed_id,
				'op_endpoint'      => $Openid_identifiers['server_url'],
