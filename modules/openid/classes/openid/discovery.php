 ...
 *
 * $Id: discovery.php 2008-09-25 17:28:07 BST Atomless $
 *
 ...
 */
abstract class Openid_Discovery extends Openid_Request {
	...
	protected function discover_xri($claimed_id, $service_types)
	{
		...
		$service_endpoints = openid_service_xrds::get_filtered_and_prioritized_service_endpoints($services, $canonical_id, $claimed_id);
	...
	protected function discover_yadis($claimed_id, $service_types)
	{
		...
		else
		{
			...
			{
				$yadis_result['xrds_url'] = openid_service_html::get_yadis_xrds_location_from_html_meta_tag_httpequiv($this->response_body);
			}
		...
		$service_endpoints = openid_service_xrds::get_filtered_and_prioritized_service_endpoints($this->xrds->service_list, $yadis_result['normalized_url'], $claimed_id);
	...
	protected function discover_html($claimed_id)
	{
		...
		$service_endpoints = openid_service_html::get_openid_service_endpoints($this->response_body, $claimed_id);
