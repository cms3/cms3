 ...
 *
 * $Id: association.php 2008-09-25 17:28:07 BST Atomless $
 *
 ...
 */
abstract class Openid_Association extends Openid_Discovery {
	...
	protected function associate()
	{
		...
		// method of the Openid_Relyingparty class) when the response_nonce is added as a cache tag.
	...
	protected function form_new_trust_association()
	{
		...
		if ($this->session_mode == Openid::OPENID_STATEFULL)
		{
			...
			$this->set_authentication_fields(diffiehellman::get_exchange_keys());
		}
		...
		$association_fields = kvf::string_to_array($this->response_body);
		...
		if (empty($association_fields['mac_key']))
		{
			...
			$association_fields['mac_key'] = diffiehellman::compute_mac_key
										   (
	...
	*/
	protected function request_association($attempts = 2)
	{
		...
		if (($this->curl_request_info['http_code'] == 400
		   ...
		   AND $attempts > 0)
		{
			$response_fields = kvf::string_to_array($this->response_body);
